...
def procesar_productos_y_nodos(conn_str, similarity_threshold, fut_similarity_threshold):
    with pyodbc.connect(conn_str) as conn:
        ...
        # Utilizar TfidfVectorizer para calcular la similitud de coseno utilizando el mismo vocabulario
...
conn_str = (
    ...
    r'DBQ=C:\Users\decanatura.ingenieri\Desktop\SonidoSISPRO\ParaPython(2)1.accdb;'
)
...
# Iterar sobre diferentes umbrales de similitud y generar gráficos
for similarity_threshold in [ 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
    for fut_similarity_threshold in [ 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
        dfdatos, df_fut_prod_x_nodo, score, producto_df = procesar_productos_y_nodos(conn_str, similarity_threshold, fut_similarity_threshold)
        if score[0] > 0 and score[1] > 0:
            if score[0] > best_score[0] or (score[0] == best_score[0] and score[1] > best_score[1]):
                best_score = score
                best_combination = (similarity_threshold, fut_similarity_threshold)
                best_dfdatos = dfdatos
                best_df_fut_prod_x_nodo = df_fut_prod_x_nodo
                best_producto_df = producto_df

print(f"La mejor combinación de umbrales es: {best_combination} con una puntuación de: {best_score}")

# Guardar la mejor combinación en un archivo Excel
if best_dfdatos is not None and best_df_fut_prod_x_nodo is not None:
    with pd.ExcelWriter('Mejor_Iteracion.xlsx') as writer:
        ...
        summary_df = pd.DataFrame({
            'Similarity Threshold': [best_combination[0]],
            'Future Similarity Threshold': [best_combination[1]]
        })
        summary_df.to_excel(writer, sheet_name='Thresholds', index=False)
